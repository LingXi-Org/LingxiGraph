...
| POST | `/v1/runs/{run_id}/steer` | Durably inject mid-run input |
...
stateDiagram-v2
  ...
  pending --> cancelled: cancel (no live worker)
  paused --> cancelled: cancel (no live worker)
  failed --> pending: redrive
  dead_letter --> pending: redrive
  state "new run: pending" as resumed_pending
  paused --> resumed_pending: resume creates descendant run
```

`resume` does not mutate the old run from `paused` back to `pending`: the old run remains `paused` and is marked stale through the typed `superseded_by_run_id` field; the newly created descendant run is the one that starts in `pending`.

Stable status values are `pending`, `running`, `paused`, `succeeded`, `failed`, `cancelling`, `cancelled`, `timed_out`, and `dead_letter`.

<Cards>
  <Card title="Authentication and RBAC" icon="key" href="/docs/en/api/authentication">Bearer tokens, development API keys, and roles.</Card>
  <Card title="Errors and SSE" icon="triangle-exclamation" href="/docs/en/api/errors-events">Stable error codes, resume, and deduplication.</Card>
</Cards>
