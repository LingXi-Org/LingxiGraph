...
outcome (`succeeded`, `paused`, `failed`, etc.) — it is **not** forced through a retry loop just
because a steering event happened to exist.
...
disposition — that is what the mechanism above guarantees. It does **not** mean no terminal run
can ever have a `pending` steering row for any reason at all: a run that exhausts delivery
attempts and is dead-lettered (or fails outright) can legitimately leave steering that the
graph simply never got around to draining still `pending` — that is deliberate, retained
durable history, not a bug, and a later `/redrive` of that run (allowed only from `failed` or
`dead_letter`) picks the still-pending row back up like any other queued steering. An ordinary
client-initiated **cancel** is different, and not currently covered by that same recovery path:
`/redrive` does not accept a `cancelled` run, so steering left `pending` on a cancelled run has
no automatic future delivery attempt today — recovering it, if ever needed, is an operator/API
task outside this mechanism, not something the Runtime resolves on its own. The two disposition
cases above are different from cancel: the boundary-race/non-consumption disposition exists
because that specific steering event durably has *no delivery attempt left, ever* that could
consume it; a `failed`/`dead_letter` run's unconsumed steering has a legitimate future delivery
path (`/redrive`) that a `superseded` disposition would incorrectly foreclose, which is why it
is intentionally left `pending` instead.
