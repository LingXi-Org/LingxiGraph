...
resumed (its still-pending steering migrates onto the new run) or because it durably landed
after the owning worker's last safe point closed (a narrow race; see "Finalization and the
steering admission boundary" below). Treat `delivered` as a status your code may need to read/allow
...
This still leaves one narrow race: a `/steer` call can win the run row's lock, and durably
commit its row, a moment *before* `close_steering()` wins that same lock — even though the
graph has already produced its terminal output and the heartbeat loop that would have
ingested the event has already stopped. Once the final flush completes, the Runtime checks for
exactly this: any steering row still `pending`/`delivered` at that point is durably
transitioned to `superseded` (reason `late_boundary_race`), paired with a `run.steer.superseded`
lifecycle event, in the same lease/attempt-fenced write as the rest of finalization. The run
then finalizes normally with its already-computed outcome (`succeeded`, `paused`, `failed`,
etc.) — it is **not** forced through a retry loop just because a steering event happened to
exist. A graph that legitimately never calls `drain_steering()` at all (a documented, allowed
choice — see "Safe points are application-chosen" above) is never penalized by this: it
finalizes on its first attempt exactly as if the steering event had never arrived, and that
event's disposition is recorded as `superseded`, not silently dropped.

**Final disposition policy, stated precisely:** a terminal run's *own* finalization decision
can never silently strand a race-losing steering event without a durable, observable
disposition — that is what the mechanism above guarantees. It does **not** mean no terminal run
can ever have a `pending` steering row for any reason at all: an ordinary client-initiated
cancel (or a run that exhausts delivery attempts and is dead-lettered) can legitimately leave
steering that the graph simply never got around to draining still `pending` — that is
deliberate, retained durable history, not a bug, and a later redrive of that run picks the
still-pending row back up like any other queued steering. The two cases are different: the
boundary-race disposition above exists because that specific steering event durably has *no
delivery attempt left, ever* that could consume it; an ordinary cancel's unconsumed steering
still has a legitimate future delivery path (redrive) that a `superseded` disposition would
