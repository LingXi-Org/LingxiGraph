...
`pending` → `consumed` directly (via a single atomic `commit_steering_consumptions()` call),
or `superseded` on transfer. Treat `delivered` as a status your code may need to read/allow
...
- **This is not a system-wide exactly-once guarantee.** If a worker crashes (or its lease is
  ...
  `commit_steering_consumptions()` durably records that consumption, the PostgreSQL row is
...
- The consumed-commit (`commit_steering_consumptions()`) and the graph's own checkpoint commit
