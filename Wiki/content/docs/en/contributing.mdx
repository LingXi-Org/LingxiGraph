...
The bilingual Fumadocs source is in `Wiki/content/docs/`:
...
Every user-facing page keeps the same relative path under `content/docs/zh/` and `content/docs/en/`. Update both languages in the same PR as a code/API change. Examples must be copyable; development-only authentication and dangerous operations must be called out explicitly.
