...
The China endpoint defaults to `https://api.coze.cn`. Set `base_url` explicitly for the international service, Coze Studio, or a shared gateway. In production, retrieve rotating credentials through a sync/async `token_provider`; never store them in state, context, or checkpoints.
