...
| POST | `/v1/runs/{run_id}/steer` | durably 注入运行中输入 |
...
stateDiagram-v2
  ...
  pending --> cancelled: cancel（无存活 worker）
  paused --> cancelled: cancel（无存活 worker）
...
`resume` 不会把旧 run 的 `paused` 状态改成 `pending`：旧 run 仍保持 `paused`，并通过类型化的 `superseded_by_run_id` 标记已被取代；新创建的后继 run 才从 `pending` 开始执行。

稳定状态值为 `pending`、`running`、`paused`、`succeeded`、`failed`、`cancelling`、`cancelled`、`timed_out`、`dead_letter`。

<Cards>
  <Card title="认证与 RBAC" icon="key" href="/docs/zh/api/authentication">Bearer token、开发 API key 与角色。</Card>
  <Card title="错误与 SSE" icon="triangle-exclamation" href="/docs/zh/api/errors-events">稳定错误码、续传与去重。</Card>
</Cards>
