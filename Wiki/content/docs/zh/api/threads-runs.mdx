...
idempotency_key)` 三元组只会创建一条 steering 事件——重复调用返回**原**事件（不是
409），响应体与首次调用完全一致，`sequence` 也不会递增。这与 `POST /runs` 创建接口的
`Idempotency-Key` 语义一致，都是"同 key 同请求安全重放"，而不是"报错"。
...
| `succeeded` / `failed` / `cancelled` / `timed_out` / `dead_letter`（终止态） | `409 run_terminal`，不产生任何事件 |
...
（`run.steer.accepted`，状态 `pending`）——**不代表图已经处理这条输入**。Redis 只用于
可选的低延迟 `run.steer.available` 通知，从不作为唯一数据源；即使 Redis 通知丢失，
worker 仍会通过 PostgreSQL 轮询/安全点检查最终发现并消费该事件。只有当某个安全点的
图代码真正调用 `runtime.drain_steering()` 读取到该事件，并且这次消费被
`commit_steering_consumptions()` durably 提交后，才会出现 `run.steer.consumed`。
调用方不能仅凭 202 就假设图已经"看到"这条输入；需要观察 `run.steer.consumed`
事件（或轮询 `GET /v1/runs/{run_id}/steer` 列表的 `status` 字段）来确认真正被消费。
