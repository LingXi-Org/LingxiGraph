...
新 run），也可能是它在所属 worker 的最后安全点关闭之后才 durably 落地（一种狭窄的竞争；
见下文"终结流程与 steering 准入边界"一节）。请把 `delivered` 当作代码里
...
但这仍然留下一种狭窄的竞争：某次 `/steer` 调用可能恰好在 `close_steering()` 抢到同一把
...
lease/attempt-fenced 写入中一起完成。随后该 run 会以之前已经算出的结果
（`succeeded`、`paused`、`failed` 等）正常终结——**不会**仅仅因为存在一条 steering 事件
就被强制进入重试循环。一个本就合法地从不调用 `drain_steering()` 的图（见上文"安全点由
应用选择"，这是文档明确允许的选择）不会因此受到惩罚：它会在第一次投递尝试就正常终结，
就像这条 steering 事件从未到达过一样，而该事件的最终归宿会被记录为 `superseded`，绝不会
被静默丢弃。

**最终归宿策略，精确表述如下：** 一个 terminal run **自身**的终结决策绝不能让一条输了竞争
的 steering 事件被静默搁置、没有任何 durable、可观测的归宿——这正是上面机制所保证的。但这
**并不**意味着任何 terminal run 在任何情况下都不可能留有 `pending` 的 steering 行：一次普通
的、由客户端发起的 cancel（或者一个耗尽投递尝试次数而被 dead-letter 的 run）完全可能合法地
留下图根本没来得及消费的、仍是 `pending` 的 steering——这是刻意保留的 durable 历史记录，而
不是 bug；之后如果该 run 被 redrive，这条仍 pending 的行会像任何其他排队中的 steering 一样
被重新拾起。这两种情况是不同的：上面的边界竞争归宿之所以存在，是因为那条具体的 steering
事件确实**再也不会有任何投递机会**去消费它；而一次普通 cancel 未被消费的 steering 仍然拥有
一条合法的未来投递路径（redrive），如果给它打上 `superseded` 反而会错误地堵死这条路径。
