...
**投递**本身——durable、有序、去重、恰好一次的 drain——但从不替业务决定"这条新输入意味着
什么"。一条消息应该触发重新规划、只是追加到上下文，还是应该被忽略（因为已经跑过那一步了），
完全是业务图的逻辑。
...
sequenceDiagram
  ...
  W->>PG: commit_steering_consumptions()（状态更新 + run.steer.consumed，同一事务）
```

PostgreSQL 的 `run_steering_events` 表是每一条 steering 事件是否存在及其状态
（`pending` → `delivered` → `consumed`，迁移时为 `superseded`）的唯一真相来源。Redis
...
- `drain_steering()` 按该顺序返回事件，且永远不会返回同一事件两次——即使经历重试或
  worker 重启，drain-once 也是原子的；
...
- **worker 重试：** `commit_steering_consumptions()` 是幂等的——一个 worker 提交某批
...
`commit_steering_consumptions()`）与图自身为消费它的那个 superstep 所做的 checkpoint
