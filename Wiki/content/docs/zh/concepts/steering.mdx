...
随后该 run 会以之前已经算出的结果（`succeeded`、`failed`、`dead_letter` 等——任何终态；
`paused` 不属于此终结路径，因为它本来就从来不是一种终态，见下文）正常终结——**不会**仅仅
因为存在一条 steering 事件就被强制进入重试循环。
...
的 steering 事件被静默搁置、没有任何 durable、可观测的归宿——这正是上面机制所保证的。任何
真正终结的 `finish` 意图——由图自身的结果计算产生的 `succeeded`、`failed`、`cancelled` 或
`dead_letter`——都会封闭 steering 准入并 supersede 遗留行，与上文描述完全一致；唯一被排除
在外的是 `paused`（见下一段）。也就是说，一次普通的 `failed`/`dead_letter` 结果本身**不会**
让 steering 停留在 `pending`。

唯一仍可能出现这种情况的场景比"被 dead-letter 的 run 就会留下 pending steering"要窄得多：
特指**最终 steering flush 本身就无法持久提交**的那次 run（`_retry_or_dead_letter()`——见
`Worker._execute` 里 final flush 之后的处理）。这条路径根本不会走到
`finalize_run_with_steering_disposition_if_owned()`——它会转而让这次投递尝试走普通的
retry/dead-letter 流程，因为此时写下任何归宿（无论是否 superseded）都会要么报告一个虚假
状态，要么静默丢失已经 drain 但尚未提交的消费记录。只有在这种特定情况下，一个 `dead_letter`
的 run 才可能仍然留有 `pending` 的 steering——这是刻意保留的 durable 历史记录，而不是 bug：
之后如果该 run 被 `/redrive`，这条仍 pending 的行会像任何其他排队中的 steering 一样被重新
拾起。一次普通的、由客户端发起的 **cancel** 不在这条恢复路径之内：`/redrive` 不接受
`cancelled` 的 run，因此留在 `cancelled` run 上的 `pending` steering 目前没有任何自动的
未来投递机会——如果确实需要恢复，那是这套机制之外的运维/API 任务，而不是由 Runtime 自动
解决的。
