...
- 升级兼容性：升级到当前版本之后，一个升级前就存在、恰好携带同名用户 metadata key 的历史
...
- 因为没有 schema 回滚路径，`0002_steering`/`0003_steering_source_event`/
  ...
  `test_postgres_integration.py` Postgres job）中验证过，再应用到生产环境，而不是依赖
  "出问题了还能撤销"。
