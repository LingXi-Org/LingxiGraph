...
`Command(scope=PARENT)` handoff 至父图；状态查询和事件 envelope 保留 namespace，调用方
可按需折叠或展开。

## 流与事件

流模式统一为 `values`、`updates`、`events`、`custom` 和 `messages`。Event envelope 包含
...
- `async`：使用异步 checkpointer 接口；当前服务器仍在每步边界等待持久化确认，以保持
  PostgreSQL 事务与事件顺序一致。
