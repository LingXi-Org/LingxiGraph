...
| `LINGXIGRAPH_LOG_LEVEL` | 日志级别，默认 `INFO` |
| `LINGXIGRAPH_LOG_FORMAT` | `json`（默认）或文本 |
...
默认保护上限为每 tenant 600 request/min、1 MiB request、1 MiB checkpoint state 和
256 KiB event；可按容量模型通过 `RepositoryLimits` 收紧。run 还应设置模型/工具/token/cost
预算，避免循环提示或故障 provider 无界消耗。
...
Worker 健康端口默认 `8125`：`/health` 表示事件循环存活，`/ready` 在 drain 期间返回 503。
API `/ready` 同时检查至少一个 graph、repository `SELECT 1` 和 embedded worker 状态。Helm
Chart 已配置 startup/readiness/liveness probes 和 preStop/drain 窗口。

dead-letter 排障步骤：读取 run `error` 与 `worker_retrying` 事件，修复下游或配置，确认图版本
仍在 registry 后调用 `POST /v1/runs/{id}/redrive`。redrive 保留原 graph/version/input/config，
不要用它替代需要新输入或新图版本的正常新 run。
